#!/usr/bin/env python
# ssh.py - wrapper to launch saved sessions with putty
...
def usage():
	be.cprint("white","Wallix SSH launcher\n")
	be.cprint("White","-------------------\n\n")
	be.reset_colour
...
def searchdb(db,findstr,findstr2=''):
	...
	be.cprint("white", "{0:^15s} | {1:^10s} | {2:^8s} | {3:^10s} | {5:^30s} | {6:^14s}|\n".format("Server", "Customer","Env","Type","User","Connection String","NAT","Description"))
	be.cprint("white", "{0:<15s}-|-{1:<10s}-|-{2:<8s}-|-{3:<10s}-|-{5:<30s}-|-{6:<14s}|\n".format("-" *15, "-"*10,"-"*8,"-"*10,"-","-"*30,"-"*14,"-"*30))
	for i in db:
...
def launch(db,server,user=''):
	...
	for i in db:
		...
		if re.search(server,recstr):
			#Launch and exit
			#cmd=[ 'terminator','-e ssh',i.split('|')[4] + '@' + i.split('|')[5] + i.split('|')[6],'2>&1 >/dev/null' ]
			if i.split('|')[2] == 'PROD': 
				profile='Production'
			elif i.split('|')[2] == 'DR': 
				profile='DR'
			...
			if sys.platform == "linux" or sys.platform == "linux2":
				print "TBA"
				cmd=['/usr/bin/putty', '-load', profile, user + '@' + i.split('|')[5] + '@' + i.split('|')[6] ]
			elif sys.platform == "win32":
				cmd=[ 'putty','-load', profile, user + '@' + i.split('|')[5] + '@' + i.split('|')[6] ]
