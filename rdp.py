#!/usr/bin/env python
# rdp.py - wrapper to launch wallix rdp sessions
...
dbfile=r'rdplist.csv'
debug=0

def usage():
	be.cprint("white","Wallix RDP launcher\n")
	be.cprint("White","-------------------\n\n")
	be.reset_colour
	...
	for i in db:
		...
		if re.search(server,recstr):
			...
			if sys.platform == "linux" or sys.platform == "linux2":
				...
				f=open('wallix.rdp','w')
				f.write("full address:s:wallix\n")
				f.write("username:s:"+conn+"\n")
				f.write("drivestoredirect:s:*\n")
				f.close()
				cmd=[ 'start','wallix.rdp' ]
